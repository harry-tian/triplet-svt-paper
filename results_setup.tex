...
In this section, we review the specific model instantiation in our experiments and provide the detailed experimental setup for justification and decision support.
...
Following \citep{chen2020simple}, we take the output of the average pooling layer and feed it into an MLP projection head with desired embedding dimension. We use the output of the projection head as our final embeddings, where we add task-specific head and loss for training and evaluation.
% We project with either the original 512-dimension final layer 
% \chenhao{update the language, han} 
% or projecting it to a 50-dimension layer 
% \chenhao{now I am not even sure that this is correct.} 
% because lower dimensions have been shown more effective in metric learning 
% \chenhao{han, do you have a citation?}.
We use Euclidean distance as the similarity metric for both loss calculation and distance measurement during example selection in decision support. All similarity measurement and losses calculation are computed using the final embeddings.
...
Our first baseline is \resn fine-tuned with classification data using cross-entropy loss; we simply call it \resn moving forward.
\resn typically achieves high classification accuracy but does not necessarily produce human-aligned representations; as shown by \citet{roads2020}, the alignment is even worse with more parameters.
Our second baseline model is the same pretrained model finetuned with human triplets using triplet margin loss~\citep{balntas2016learning}; we call it \tn.
...
Empirically tuning $\lambda$ confirms this hypothesis (see the supplementary materials). For the main paper, we present results with
one value of $\lambda$ for each experiment. More details about hyperparameter tuning can be found in the supplementary materials.
...
our main evaluation is designed to simulate case-based decision support scenarios.
...
% We assume that the classification problem is binary moving forward and discuss how our method and evaluation can generalize to more than two classes in \shi{probably supplementary materials?}.
