...
\caption{\harry{I removed a few sentences here} Nearest neighbor retrieved by the model representation might not align with human similarity judgment. 
% The \resn embeddings (512-dim) are visualized using t-SNE \cite{van2008visualizing}. 
% The purple circle represents a specific test instance.
%  The nearest neighbor found by \resn (pink circle) is not as visually similar 
% to human perception 
% as the instance in cyan circle found by optimizing a metric learning objective.
The nearest neighbor found by \resn  is not as visually similar as the instance  found by optimizing a metric learning objective.
}
\label{fig:model_vs_human}
\end{figure}
...
Specifically, we use human triplet judgments (i.e., choosing which example is more similar to a reference example among two candidates) to capture human visual perception related to a classification task, and supplement standard classification objectives with a triplet loss~\cite{balntas2016learning}.
...
\begin{itemize}[leftmargin=*, noitemsep, topsep=-1.5pt]
  \item We highlight the importance of alignment in learning human-compatible representations for case-based decision support.
  \item We propose a multi-task learning framework that combines supervised learning and metric learning.
  \item Empirical results with synthetic data and human subject studies demonstrate the effectiveness of our approach.
\end{itemize}
>>>>>>> d826d84d512431b06153b122f8f5b2a3ff0b5e92
