...
We conduct human subject experiments on a natural image dataset: Butterflies v.s. Moths (BM). Following \cite{singla2014near}, we acquired 200 images from the publicly available dataset set (ImageNet \cite{krizhevsky2012imagenet}). We present synthetic experiment results in the appendix.
% \harry{we can also remove these 2 sentences?} 
BM is a binary classification problem and each class contains four species (see appendix for examples). We randomly split the dataset following 60\%:20\%:20\% ratio (120/40/40 images).
