...
We conduct human subject experiments on a natural image dataset: Butterflies v.s. Moths (BM). Following \citet{singla2014near}, we acquired 200 images from the publicly available dataset set (ImageNet \cite{krizhevsky2012imagenet}). 
% \harry{we can also remove these 2 sentences?} 
BM is a binary classification problem and each class contains four species (see appendix for examples). We randomly split the dataset following 60\%:20\%:20\% ratio (120/40/40 images). We also present synthetic experiment results in the appendix.


\subsection{Triplet annotation}

We recruit 80 crowdworkers on Prolific to acquire visual similarity triplets. In each question for the annotator, we show the reference image on top and two candidate below, and ask 2-Alternative-Forced-Choice (2AFC) question: which candidate image looks more similar to the reference image. 
% \harry{can prob remove the next sentence?} 
An screenshot of the interface can be found in the appendix.
...
\begin{table}[t]
    ...
    % \end{minipage}
    % \hfill
    % \han{syn decision tbl here is removed}
    ...
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=0.65\textwidth]{figures/bm/prolific/decision.pdf}
...
We recruit crowdworkers on Prolific to evaluate representations produced by our models in decision support tasks.
% by doing decision support tasks and H2H comparison tasks. 
% \han{Removed H2H comparison task here.}
We acquired examples with different policies from two sets of model representations: the filtered 50-dimension representations \mtl and the 512-dimension \resn baseline representations. 
