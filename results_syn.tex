...
We report the results for \figref{fig:vw}a in the main paper; see the supplementary materials for full results.
...
See supplementary materials for more details on how we generate these weights.
In each setting, we generate 40,000 triplets using the simulated humans similarity metric.
% \chenhao{explain the choice of weights in more details and refer to the supplementary materials.}
...
% \para{Hyperparameters. } For VW we oberve that using a 512-dimension embedding provides better performance across all evaluation metrics and all models. Other details can be found in supplementary materials.
% \harry{do we need more here? batch size, learning rate etc.}
% \chenhao{not here, but we should say it at other places and include all these in the supplementary materials.}
% \harry{where's a good place?}\cc{supplementary materials}
...
% \mtl achieves high classfication accuracy compared to RESN and high triplet accuracy compared to TN (results are included in the supplementary materials). 
% \cc{we need to include this result in the main paper not supplementary materials (?)}
...
$\lambda$'s role is relatively limited and we will discuss its effect in supplementary materials.
...
% More detailed results on \mtl classification and triplet accuracies are included in the supplementary materials.
